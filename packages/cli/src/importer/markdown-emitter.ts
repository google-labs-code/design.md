...
function buildFrontmatter(state: DesignSystemState): string {
...
function overviewSentence(state: DesignSystemState, ctx: EmitContext | undefined): string {
  ...
  return `This DESIGN.md was imported from ${projectKind} and contains ${counts.join(', ')}.`;
}

function sourceSummary(sources: ScanResult): string | null {
  const parts: string[] = [];
  if (sources.tailwindConfigs.length > 0) {
    parts.push(pluralize(sources.tailwindConfigs.length, 'tailwind config', 'tailwind configs'));
  }
...
function buildBody(state: DesignSystemState, ctx?: EmitContext): string {
  ...
  lines.push('## Overview');
  lines.push('');
  lines.push(overviewSentence(state, ctx));
  if (ctx?.sources) {
  ...
  lines.push('---');
  lines.push('');
  lines.push('_Generated by `design.md import`. Edit this body to add rationale, context, and usage guidelines — the frontmatter is the machine-readable source of truth._');
  lines.push('');
  return lines.join('\n');
}
 ...
 */
export function emitDesignMd(state: DesignSystemState, ctx?: EmitContext): string {
