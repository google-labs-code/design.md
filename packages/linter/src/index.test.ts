...
`;
    ...
    // ── Tailwind assertions ─────────────────────────────────────────
    expect(result.tailwindConfig.theme.extend.colors?.['primary']).toBe('#647d66');
    expect(result.tailwindConfig.theme.extend.fontFamily?.['headline-lg']).toContain('Google Sans Display');
    expect(result.tailwindConfig.theme.extend.borderRadius?.['full']).toBe('9999px');
    expect(result.tailwindConfig.theme.extend.spacing?.['gutter-s']).toBe('8px');
  });
