...
It is a map\<string, Color | Color Group>, that maps the name of the color token (or group) to its value.

Tokens defined in nested groups are automatically flattened. For example, `utility-info` with a nested `50` key becomes the `utility-info-50` color token and can be referenced via `{colors.utility-info-50}` or `{colors.utility-info.50}`.
